...
class Player extends Entity {
   ...
   * @param {Vector} position The player's starting location, generated inside world.
   * @param {number} angle The player's sprite angle to start with (0)
   */
  ...
  updateOnInput(data) {
    if (data.up) {
      this.velocity = Vector.fromPolar(this.speed, this.spriteAngle)
    } else if (data.down) {
      this.velocity = Vector.fromPolar(-this.speed, this.spriteAngle)
  ...
  update(lastUpdateTime, deltaTime) {
    ...
    this.boundToWorld()
    this.spriteAngle = Util.normalizeAngle(
      this.spriteAngle + this.turnRate * deltaTime)
